...
// ---- protected sample
app.get('/api/ping', verifyBearer, (req, res) => {
    res.json({
        ok: true,
        sub: req.auth?.sub,
...
// ---- users init/me
app.post('/api/users/init', verifyBearer, async (req, res) => {
    ...
    on conflict (subject) do update set
      ...
      updated_at = now()
  `, [sub, email, name]);
    await q(`insert into app_user_audit (subject, action, details)
     values ($1, 'created', jsonb_build_object('email',$2,'name',$3))`, [sub, email, name]);
    const [me] = await q(`select status from app_user where subject=$1`, [sub]);
    res.json({ ok: true, status: me?.status || 'pending' });
});
// ---- admin approve/reject (email guard for now)
const ADMIN_EMAIL = (process.env.ADMIN_EMAIL || '').toLowerCase();
app.post('/api/admin/users/:subject/approve', verifyBearer, async (req, res) => {
...
app.post('/api/admin/users/:subject/reject', verifyBearer, async (req, res) => {
    if ((req.auth?.preferred_username || '').toLowerCase() !== ADMIN_EMAIL) {
        return res.status(403).json({ error: 'forbidden' });
    }
    const subject = req.params.subject;
...
app.get('/api/admin/users', verifyBearer, async (req, res) => {
    if ((req.auth?.preferred_username || '').toLowerCase() !== ADMIN_EMAIL) {
        return res.status(403).json({ error: 'forbidden' });
    }
    const status = req.query.status || '';
    const rows = status
        ? await q(`select subject, email, display_name, status, created_at, updated_at
